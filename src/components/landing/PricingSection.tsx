...
function FeatureItem({ feature }: { feature: PlanFeature }) {
  const content = (
    <span className="flex items-start gap-2 text-sm">
      ...
      {feature.tooltip && <Info className="h-3.5 w-3.5 mt-0.5 shrink-0 text-muted-foreground/50" />}
    </span>
  );
...
function PlanCard({ plan, period }: { plan: PlanData; period: BillingPeriod }) {
  ...
  return (
    <div
      className={`relative flex flex-col rounded-2xl border p-6 lg:p-8 transition-all ${
        plan.recommended
    ...
    >
      ...
      <div className="mb-1 flex items-end gap-2 flex-wrap">
        <span className="text-4xl font-bold text-foreground">{plan.getPrice(period)}</span>
      ...
      <Button
        ...
        size="lg"
        className={`w-full text-sm ${plan.premium ? "bg-foreground text-background hover:bg-foreground/90" : ""}`}
        asChild
      >
    ...
    <>
      <section id="pricing" className="bg-card border-y border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-20">
          ...
          <div className="text-center mb-12">
            <Badge variant="secondary" className="mb-3 text-xs">💰 Transparent. Sans surprise.</Badge>
            ...
            <div className="inline-flex items-center bg-muted rounded-full p-1 gap-1">
              {BILLING_OPTIONS.map((opt) => (
                ...
                >
                  {opt.label}
                  {opt.badge && (
                    <span className="ml-1.5 text-[10px] text-emerald-600 dark:text-emerald-400 font-bold">{opt.badge}</span>
                  )}
                </button>
              ))}
            </div>
          ...
          <div className="rounded-2xl border border-dashed border-border bg-muted/30 p-6 lg:p-8 flex flex-col md:flex-row items-start md:items-center gap-6">
            ...
            <div className="flex-1">
              <div className="grid sm:grid-cols-2 gap-x-6 gap-y-2">
                {CUSTOM_FEATURES.map((f) => (
                  <span key={f} className="flex items-start gap-2 text-sm">
                    <Check className="h-4 w-4 mt-0.5 shrink-0 text-emerald-500" />
                    <span>{f}</span>
                  </span>
                ))}
          ...
          <div className="flex items-center justify-center gap-4 mt-8 text-muted-foreground/50">
            ...
            <span className="text-xs">Paiement sécurisé — Visa · Mastercard · CB</span>
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-border bg-muted/30">
        <div className="max-w-4xl mx-auto px-6 py-16 text-center space-y-6">
          <h2 className="text-3xl md:text-4xl">Quel que soit votre plan, commencez aujourd'hui.</h2>
